...
def epsilon_greedy(state_vector, epsilon):
    ...
    if np.random.random() < epsilon:
        ...
        action_state, obj_state = model(state_vector)        
        action_index, object_index = action_state.argmax(), obj_state.argmax()
...
def deep_q_learning(current_state_vector, action_index, object_index, reward,
    ...
    # TODO Your code here
    # Loss(L)= 1/2 * (y-Q(s,c,theta))^2
    # y = reward + GAMMA * max Q(s',c',theta)

    maxq_next = 0 if terminal else maxq_next
    y = reward + GAMMA * maxq_next
    q_value = 1 / 2 * (q_value_cur_state[0][action_index] + q_value_cur_state[1][object_index])
    
    # taking a gradient step with respect to the squared loss 
    loss = 0.5 *(y - q_value)**2    
