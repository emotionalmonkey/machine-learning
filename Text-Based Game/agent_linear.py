...
def epsilon_greedy(state_vector, theta, epsilon):
    ...
    if np.random.random() < epsilon:
        ...
        max_index = np.argmax(theta @ state_vector)
...
def linear_q_learning(theta, current_state_vector, action_index, object_index,
    ...
    theta_c = theta[tuple2index(action_index, object_index)] 
    theta[tuple2index(action_index, object_index)] = theta_c + ALPHA * (reward + GAMMA * max_Q - q_value) * current_state_vector   
...
if __name__ == '__main__':
    game_path = path.join(path.dirname(__file__),"game.tsv")
    state_texts = utils.load_data(game_path)
    ...
    axis.set_title(('Linear: nRuns=%d, Epilon=%.2f, Epi=%d, alpha=%.4f' % (NUM_RUNS, TRAINING_EP, NUM_EPIS_TRAIN, ALPHA)))
    plt.show()
